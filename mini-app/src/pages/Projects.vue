<template>
  <div class="space-y-8">
    <!-- Hero Section -->
    <div class="relative overflow-hidden rounded-[2rem] md:rounded-[2.5rem] bg-gradient-to-br from-emerald-600 to-teal-700 text-white p-5 md:p-10 shadow-2xl mb-6 md:mb-10 sticky top-0 md:relative z-30">
      ...
      <div class="relative z-10">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6 mb-6 md:mb-8">
          <div>
            <h1 class="text-2xl md:text-4xl font-black tracking-tight mb-1">Projects</h1>
            <p class="text-emerald-100 text-xs md:text-base font-medium opacity-90">Organize your work into projects.</p>
          </div>
          <button
            @click="showNewProjectModal = true"
            class="bg-white text-emerald-600 px-4 md:px-6 py-2 md:py-3 rounded-xl md:rounded-2xl font-bold hover:bg-emerald-50 transition-all flex items-center justify-center space-x-2 shadow-xl shadow-emerald-900/20 group text-xs md:text-base"
          >
            <svg
        ...
        <div class="flex items-center space-x-3 overflow-x-auto pb-2 md:pb-0 no-scrollbar mb-4 md:mb-6">
          <div class="bg-white/10 backdrop-blur-md border border-white/20 px-3 md:px-5 py-1.5 md:py-2.5 rounded-xl md:rounded-2xl flex items-center space-x-2 md:space-x-3 whitespace-nowrap">
            <span class="text-base md:text-2xl font-black">{{ totalProjects }}</span>
            <span class="text-[8px] md:text-xs font-bold uppercase tracking-widest opacity-80">Total</span>
          </div>
          <div class="bg-white/10 backdrop-blur-md border border-white/20 px-3 md:px-5 py-1.5 md:py-2.5 rounded-xl md:rounded-2xl flex items-center space-x-2 md:space-x-3 whitespace-nowrap">
            <span class="text-base md:text-2xl font-black text-emerald-300">{{ activeProjects }}</span>
            <span class="text-[8px] md:text-xs font-bold uppercase tracking-widest opacity-80">Active</span>
          </div>
          <div class="bg-white/10 backdrop-blur-md border border-white/20 px-3 md:px-5 py-1.5 md:py-2.5 rounded-xl md:rounded-2xl flex items-center space-x-2 md:space-x-3 whitespace-nowrap">
            <span class="text-base md:text-2xl font-black text-amber-300">{{ completedProjects }}</span>
            <span class="text-[8px] md:text-xs font-bold uppercase tracking-widest opacity-80">Done</span>
          </div>
        </div>

        <!-- Quote -->
        <div v-if="projectStore.projects.length > 0" class="hidden md:block pt-6 border-t border-white/10">
          <p class="text-emerald-100/80 italic font-medium leading-relaxed max-w-2xl">
            "The secret of getting ahead is getting started."
            <span class="block text-xs font-bold uppercase tracking-widest mt-2 opacity-60">— Mark Twain</span>
          </p>
        </div>
      </div>
    </div>
