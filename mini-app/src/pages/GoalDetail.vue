<template>
  <div class="space-y-8 pb-24 md:pb-12">
    ...
    <div v-else-if="goal" class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <!-- Premium Hero Header -->
      <div class="relative overflow-hidden rounded-[2.5rem] md:rounded-[3.5rem] bg-slate-900 text-white p-6 md:p-16 shadow-2xl">
        ...
        <div class="relative z-10 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 lg:gap-12">
          <div class="space-y-6 max-w-3xl">
            <div class="flex flex-wrap items-center gap-3">
               <span class="px-4 py-1.5 rounded-full bg-white/10 text-[10px] font-black backdrop-blur-xl border border-white/10 uppercase tracking-[0.2em] text-indigo-300">Objective</span>
               <span v-if="goal.target_date" class="px-4 py-1.5 rounded-full bg-white/10 text-[10px] font-black backdrop-blur-xl border border-white/10 flex items-center gap-2 uppercase tracking-[0.2em]">
            ...
            <h1 class="text-4xl md:text-7xl font-black tracking-tighter leading-none bg-clip-text text-transparent bg-gradient-to-r from-white via-white to-white/50">{{ goal.title }}</h1>
            <p class="text-slate-400 text-base md:text-xl font-medium leading-relaxed max-w-2xl">{{ goal.description }}</p>
            
            <div v-if="goal.motivation" class="flex items-start gap-4 bg-white/5 backdrop-blur-xl rounded-[2rem] p-6 border border-white/10 max-w-xl group hover:bg-white/10 transition-all duration-500">
              <div class="w-12 h-12 rounded-2xl bg-indigo-500/20 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                <svg class="w-6 h-6 text-indigo-400" fill="currentColor" viewBox="0 0 20 20"><path d="M10.394 2.08a1 1 0 00-.788 0l-7 3a1 1 0 000 1.84L5.25 8.051a.999.999 0 01.356-.257l4-1.714a1 1 0 11.788 1.838L7.667 9.088l1.94.831a1 1 0 00.787 0l7-3a1 1 0 000-1.838l-7-3zM3.31 9.397L5 10.12v4.102a8.969 8.969 0 00-1.05-.174 1 1 0 01-.89-.89 11.115 11.115 0 01.25-3.762zM9.3 16.573A9.026 9.026 0 007 14.935v-3.957l1.818.78a3 3 0 002.364 0l5.508-2.361a11.026 11.026 0 01.25 3.762 1 1 0 01-.89.89 8.968 8.968 0 00-5.35 2.524 1 1 0 01-1.4 0z"></path></svg>
              </div>
              <div>
                <p class="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mb-1">Core Motivation</p>
                <p class="text-white italic font-medium text-lg">"{{ goal.motivation }}"</p>
              </div>
            </div>
          </div>
          ...
          <div class="relative shrink-0 group">
            <div class="absolute inset-0 bg-indigo-600/20 rounded-full blur-2xl group-hover:bg-indigo-600/40 transition-all duration-700"></div>
            <div class="relative w-40 h-40 md:w-64 md:h-64 flex items-center justify-center bg-white/5 backdrop-blur-2xl rounded-full border border-white/10 shadow-2xl">
              <svg class="w-full h-full transform -rotate-90">
                <circle cx="50%" cy="50%" r="45%" stroke="currentColor" stroke-width="8" fill="transparent" class="text-white/5" />
                <circle cx="50%" cy="50%" r="45%" stroke="currentColor" stroke-width="8" fill="transparent" class="text-indigo-500 transition-all duration-1000 ease-out" :stroke-dasharray="2 * Math.PI * 45 + '%'" :stroke-dashoffset="(2 * Math.PI * 45 * (1 - calculateProgress / 100)) + '%'" stroke-linecap="round" />
              </svg>
              <div class="absolute inset-0 flex flex-col items-center justify-center">
                <span class="text-3xl md:text-6xl font-black">{{ calculateProgress }}%</span>
                <span class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Complete</span>
              </div>
            </div>
          </div>
        </div>
